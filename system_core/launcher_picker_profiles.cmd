...
title Audion Disk Tools - Profile Folder Picker
...
if not defined PROFILE_NAME goto USAGE
...
set "POWERSHELL_EXE="
set "PICKER_RESULT=%TEMP%\audion_profile_pick_%RANDOM%_%RANDOM%.tmp"
...
echo Mask:
echo   profile filters
)
echo.
choice /C YN /N /M "Start operation? [Y/N]: "
if errorlevel 2 goto CANCELLED

if /I "%ACTION%"=="compare_profile" goto COMPARE_PROFILE
if /I "%ACTION%"=="sync_profile" goto SYNC_PROFILE
if /I "%ACTION%"=="dry_profile" goto DRY_PROFILE

echo [ERROR] Unknown profile picker action: %ACTION%
if not defined AUDION_NO_PAUSE pause
exit /b 1
...
:USAGE
echo [ERROR] Picker action or profile name was not provided.
if not defined AUDION_NO_PAUSE pause
exit /b 1
...
set /p "_AUDION_MASK=Which files do we process? Enter filename and/or extension masks separated by ;, for example: *.xxx; *xyz*.yyy; *.zzz (Enter = profile filters or all files): "
