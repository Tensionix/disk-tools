...
title Audion Disk Tools - Configured Profiles
...
set "MENU_FILE=%RUNTIME_DIR%\profiles_menu_en.txt"
set "RES_FILE=%RUNTIME_DIR%\profiles_menu_en_res.txt"
set "PATH_MENU_FILE=%RUNTIME_DIR%\profiles_path_mode_en.txt"
set "PATH_RES_FILE=%RUNTIME_DIR%\profiles_path_mode_en_res.txt"
set "PROFILE_MENU_FILE=%RUNTIME_DIR%\profiles_pick_en.txt"
set "PROFILE_RES_FILE=%RUNTIME_DIR%\profiles_pick_en_res.txt"
...
cls
echo ======================================================================
echo   AUDION DISK AUDITOR - CONFIGURED PROFILES
echo ======================================================================
echo Root:      %BASE_DIR%
...
"%FZF_CMD%" --prompt="audion@disk-auditor [PROFILES-EN] > " --pointer=">" --header="Choose a saved profile action. Backup profiles run as BACKUP-MIRROR." --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%MENU_FILE%" > "%RES_FILE%"
...
echo [2] Compare configured profile
echo [3] Run configured profile
echo [4] Dry run configured profile
echo [R] Russian profiles launcher
echo [0] Back
echo.
choice /C 1234R0 /N /M "Select: "
if errorlevel 6 exit /b 0
if errorlevel 5 set "RAW=launcher_ru" & goto DISPATCH_MAIN
...
if not exist "%DEFAULT_PROFILES%" (
  echo [ERROR] Profile config was not found:
...
echo [ERROR] Russian profiles launcher was not found.
goto RETURN_TO_MENU
...
"%FZF_CMD%" --prompt="audion@disk-auditor [PROFILE-EN] > " --pointer=">" --header="Choose a saved profile. *_backup profiles are BACKUP-MIRROR." --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%PROFILE_MENU_FILE%" > "%PROFILE_RES_FILE%"
set "PROFILE_CHOICE="
...
if not defined PROFILE_COUNT goto PICK_PROFILE_CLEANUP
call :PROMPT_TEXT PROFILE_INDEX "Profile number"
...
if defined PROFILE_INDEX_INVALID (
  echo [ERROR] Profile number must be numeric.
  goto PICK_PROFILE_CLEANUP
)
set /a PROFILE_INDEX_NUM=%PROFILE_INDEX% 2>nul
if !PROFILE_INDEX_NUM! LSS 1 (
  echo [ERROR] Profile number is out of range.
  goto PICK_PROFILE_CLEANUP
)
...
if not defined PROFILE_COUNT (
  echo [ERROR] No configured profiles were found.
)
goto :eof
...
"%FZF_CMD%" --prompt="audion@disk-auditor [PROFILE-PATHS-EN] > " --pointer=">" --header="Choose how to provide source and target folders." --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%PATH_MENU_FILE%" > "%PATH_RES_FILE%"
set "PATH_CHOICE="
...
echo [INFO] Profiles launcher smoke-check completed.
exit /b 0
...
:WRITE_MAIN_MENU
> "%MENU_FILE%" echo [01] LIST CONFIGURED PROFILES                ^| list_profiles    ^| show saved pairs from config
>>"%MENU_FILE%" echo [02] COMPARE CONFIGURED PROFILE              ^| compare_profile  ^| choose a saved pair and compare
>>"%MENU_FILE%" echo [03] RUN CONFIGURED PROFILE                  ^| sync_profile     ^| run the profile-defined mode
>>"%MENU_FILE%" echo [04] DRY RUN CONFIGURED PROFILE              ^| dry_profile      ^| preview the profile-defined mode
>>"%MENU_FILE%" echo [31] RUSSIAN PROFILES LAUNCHER               ^| launcher_ru      ^| switch to russian UI
>>"%MENU_FILE%" echo [00] BACK                                    ^| back             ^| return to caller
goto :eof

:WRITE_PATH_MODE_MENU
> "%PATH_MENU_FILE%" echo [01] MANUAL INPUT                       ^| manual           ^| type source and target paths
>>"%PATH_MENU_FILE%" echo [02] FOLDER PICKER                      ^| picker           ^| use the Windows folder picker
goto :eof
...
if not "%~3"=="" set /p "_AUDION_MASK=Which files do we process? Enter filename and/or extension masks separated by ;, for example: *.xxx; *xyz*.yyy; *.zzz (Enter = profile filters or all files): "
endlocal & (
